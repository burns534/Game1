...
float OctaveWave(int octaves, float persistance, vec3 Pos, float ratio, vec2 direction, float wavelength, float t, float phi, float k, float rf)
{
    float sum = 0.0f;
    float factor = 1.0f;
    for ( int i = 0; i < octaves; i++ )
    {
        sum += Wave(Pos, ratio, direction, wavelength, t, phi, k, rf);
        factor *= persistance;
...
vec3 WaveNorm(vec3 Pos, float ratio, vec2 direction, float wavelength, float t, float phi, float k, float rf)
{
    ...
    return -1.0f * vec3(direction.x * chain * pow(inner, k-1), -1.0f, direction.y * chain * pow(inner, k-1));
}

vec3 OctaveNorm(int octaves, float persistance, vec3 Pos, float ratio, vec2 direction, float wavelength, float t, float phi, float k, float rf)
{
    vec3 ans = vec3(0.0f);
    float factor = 1.0f;
    for ( int i = 0; i < octaves; i++ )
    {
        ans += factor * WaveNorm(Pos, ratio, direction, wavelength, t, phi, k, rf);
        factor *= persistance;
        wavelength *= persistance;
    }
    return ans;
...
//            norm = normalize(vec3(-grad.x, 1.0f , -grad.z)); //calculate normal
        position.y = 0.1625f + OctaveWave(6, 0.5f, aPos, 0.04f, Direction(1.0f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f) + OctaveWave(1, 0.5f, aPos, 0.04f, Direction(0.9f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f) + OctaveWave(1, 0.5f, aPos, 0.04f, Direction(0.5f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f);
        
        norm = normalize(OctaveNorm(6, 0.5f, aPos, 0.04f, Direction(1.0f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f) + OctaveNorm(1, 0.5f, aPos, 0.04f, Direction(0.9f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f) + OctaveNorm(1, 0.5f, aPos, 0.04f, Direction(0.5f, 1.0f), 0.2f, time, 0.8f, 2.0f, 2.0f));
