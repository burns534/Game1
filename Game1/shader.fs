...
void main()
{
    ...
    if ( water > 0.5f )
    {
        //fcolor = mix(texture(skybox, refract(-viewPos, norm, 1.33f)).rgb, texture(skybox, reflect(-viewPos, norm)).rgb, 0.5f); // for when I have sand underneath
        fcolor = mix(texture(skybox, reflect(viewPos, norm)).rgb, fcolor, 0.5f);
        lighting = vec3(ambient + 0.8f * fcolor + (delta * specular2) + (beta * specular3));
    }
    else lighting = vec3(ambient + fcolor);
