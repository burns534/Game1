...
public:
    ...
    NoiseMap(int width, int height)
    {
        ...
        for ( int y = 0; y < height + 2; y++ ) // + 2 allows for gaussian blur and clipping the egdes off later.
            ...
            {
                tempmap[y][x] = n->OctavePerlin( x * 50.34f, y * 50.34f, 0, 8, .45f, .00010f); //  appears to be optimal
    ...
    void blur()
    {
        ...
        for (int y = 1; y < h + 1 ; y ++)
            for (int x = 1; x < w + 1; x++)
            {
                ...
                sum += tempmap[y-1][x] * .125; // upper cell
                sum += tempmap[y+1][x] * .125; // lower cell
                sum += tempmap[y][x-1] * .125; // left cell
                sum += tempmap[y][x+1] * .125; // right cell
                sum += tempmap[y-1][x+1] * .0625; // upper right cell
                sum += tempmap[y+1][x+1] * .0625; // lower right cell
                sum += tempmap[y-1][x-1] * .0625; // upper left cell
                sum += tempmap[y+1][x-1] * .0625; // lower left cell
                tempmap[y][x] = sum;
                if (sum > 0.8f) tempmap[y][x] = 0.7f; // clip to [0, 0.7f];
                //std::cout << "sum: " << sum << "\n";
                /* This was bad coding practice.. */
